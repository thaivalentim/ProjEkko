...
def local_database_search(query: str) -> str:
    ...
    _, indices = index.search(np.array(query_embedding, dtype=np.float32), 3) # Busca os 3 trechos mais relevantes
...
def focused_web_search(query: str, location_string: str = "") -> str:
    ...
    try:
        with DDGS() as ddgs:
            results = list(ddgs.text(search_query, region='br-pt', max_results=5))
...
# --- FERRAMENTA DE CLIMA (INMET) ---
INMET_STATIONS = []
def load_inmet_stations():
    global INMET_STATIONS
    try:
        with open("estacoes_inmet.json", 'r', encoding='utf-8') as f:
...
def load_and_index_local_database():
    ...
    if not os.path.exists(doc_path) or not embedding_model:
